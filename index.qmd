...
background <-
  tibble::tibble(
    start = seq(lubridate::as_date("1991-01-01"),
                lubridate::as_date("1991-11-01"),
                "2 months"),
    end = seq(lubridate::as_date("1991-02-01"),
              lubridate::as_date("1991-12-01"),
              "2 months"),
...
ggplot2::ggplot(
  data = missoula %>%
    tidyr::pivot_longer(tmin:tmax,
                        names_to = "type"),
  mapping = aes(x = yday + lubridate::as_date("1990-12-31"))) +
...
missoula_start_end <-
  ...
  which(`80%` >= 28) %>%
  range() %>%
  magrittr::add(lubridate::as_date("1990-12-31"))
      ...
      .f =
        function(x){
          terra::rast(x) %>%
            ...
            tidyr::pivot_longer(-state:-county,
                                names_to = "date",
                                values_to = "tmin")
        },
      .options = furrr::furrr_options(seed = NULL)) %>%
...
counties_data <-
  dplyr::full_join(counties_tmin, counties_tmax,
                   by = dplyr::join_by(state, county, date, yday))

counties_dists <-
  counties_data %>%
  dplyr::group_by(state, county, yday) %>%
  dplyr::summarise(
    tmin = list(
      lmom::pelgum(
        lmom::.samlmu(-1 * tmin)
...
round_season <-
  function(x){
    if(any(is.na(x)))
      return(c(NA, NA))
    season <-
      x + lubridate::as_date("1990-12-31")
    if(x[1] >= x[2])
      lubridate::year(season[2]) <- lubridate::year(season[2]) + 1
...
counties_warm_season <-
  ...
  dplyr::select(state, county, start, end)
